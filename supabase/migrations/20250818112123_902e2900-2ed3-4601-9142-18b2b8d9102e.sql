...
-- 1. Drop public tables that expose sensitive data without proper RLS
DROP TABLE IF EXISTS public.public_companies;
DROP TABLE IF EXISTS public.public_products; 
DROP TABLE IF EXISTS public.public_quotes;
...
-- 8. Secure integrations table with encryption for sensitive data
ALTER TABLE integrations ADD COLUMN IF NOT EXISTS encrypted_settings bytea;

-- Create function to encrypt sensitive settings
CREATE OR REPLACE FUNCTION encrypt_integration_settings()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
DROP TRIGGER IF EXISTS encrypt_integration_settings_trigger ON integrations;
CREATE TRIGGER encrypt_integration_settings_trigger
  BEFORE INSERT OR UPDATE ON integrations
  FOR EACH ROW
  EXECUTE FUNCTION encrypt_integration_settings();
